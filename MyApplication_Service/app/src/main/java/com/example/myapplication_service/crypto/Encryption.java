...
public class Encryption {
     ...
     * @param strPlainMessage
     * @return
     */
    public static String encrypt(String strPlainMessage) {
/*
        ...
        byte[] cipherText = cipher.doFinal(encryptMessage.getBytes(StandardCharsets.UTF_8));

        ByteBuffer byteBuffer = ByteBuffer.allocate(iv.length + cipherText.length);
        byteBuffer.put(iv);
        byteBuffer.put(cipherText);
        byte[] cipherMessage = byteBuffer.array();
        Base64.decode(cipherMessage, Base64.NO_WRAP);
...
*/
        ...
        for (int i = 0; i < strPlainMessage.length(); i++) {
            // put the ascii value of
            // each character in the array
            arr[i] = (int) strPlainMessage.charAt(i);
            cu++;
        }
        String res = "";
